define(["require", "exports", "@syncfusion/ej2-grids", "@syncfusion/ej2-base", "@syncfusion/ej2-base", "@syncfusion/ej2-base", "@syncfusion/ej2-data", "@syncfusion/ej2-popups", "../base/constant", "../base/classes", "../pop-up/dialog", "../common/utility", "../common/utility", "../common/operations", "../common/index", "../common/index", "../common/index", "../common/index"], function (require, exports, ej2_grids_1, ej2_base_1, ej2_base_2, ej2_base_3, ej2_data_1, ej2_popups_1, events, CLS, dialog_1, utility_1, utility_2, operations_1, index_1, index_2, index_3, index_4) {
    "use strict";
    ...
    var DetailsView = /** @class */ (function () {
        ...
        function DetailsView(parent) {
            ...
            ej2_grids_1.Grid.Inject(ej2_grids_1.Resize, ej2_grids_1.ContextMenu, ej2_grids_1.Sort, ej2_grids_1.VirtualScroll);
            this.parent = parent;
            this.element = ej2_base_1.select('#' + this.parent.element.id + CLS.GRID_ID, this.parent.element);
        ...
        DetailsView.prototype.render = function (args) {
            ...
            if (this.parent.view === 'Details') {
                ...
                var items = utility_1.getSortedData(this.parent, args.files);
                this.checkNameWidth();
                ...
                this.gridObj = new ej2_grids_1.Grid({
                    dataSource: items,
                    allowSorting: true,
        ...
        DetailsView.prototype.checkNameWidth = function () {
            ...
            for (var i = 0; i < initialColumn.length; i++) {
                if (initialColumn[i].field === 'name') {
                    this.isNameWidth = !ej2_base_2.isNullOrUndefined(initialColumn[i].width);
                    return;
                }
            }
        };
        ...
        DetailsView.prototype.getColumns = function () {
            ...
            if (this.parent.isMobile) {
                columns = [
                    {
                        field: 'name', headerText: utility_1.getLocaleText(this.parent, 'Name'), width: 'auto', minWidth: 120, headerTextAlign: 'Left',
                ...
                for (var i = 0, len = columns.length; i < len; i++) {
                    columns[i].headerText = utility_1.getLocaleText(this.parent, columns[i].headerText);
                }
            }
        ...
        DetailsView.prototype.adjustHeight = function () {
            ...
            var gridHeader = ej2_base_1.select('.' + CLS.GRID_HEADER, this.parent.element);
        ...
        DetailsView.prototype.onRowDataBound = function (args) {
            var td = ej2_base_1.select('.e-fe-grid-name', args.row);
            if (!td) {
            ...
            if (td) {
                td.setAttribute('title', ej2_base_1.getValue('name', args.data));
            }
            if (this.parent.isLayoutChange && this.parent.isCut && this.parent.fileAction === 'move' &&
                ...
                if (this.parent.selectedNodes.indexOf(ej2_base_1.getValue('name', args.data)) !== -1) {
                    index_1.addBlur(args.row);
                }
            }
            if (!this.parent.showFileExtension && ej2_base_1.getValue('isFile', args.data)) {
                var textEle = args.row.querySelector('.e-fe-text');
                if (textEle) {
                    var name_1 = ej2_base_1.getValue('name', args.data);
                    var type = ej2_base_1.getValue('type', args.data);
                    textEle.innerHTML = name_1.substr(0, name_1.length - type.length);
                }
            }
            if (ej2_base_1.getValue('size', args.data) !== undefined && args.row.querySelector('.e-fe-size')) {
                var sizeEle = args.row.querySelector('.e-fe-size');
                var modifiedSize = void 0;
                if (!ej2_base_1.getValue('isFile', args.data)) {
                    modifiedSize = '';
                }
                else {
                    var sizeValue = ej2_base_1.getValue('size', args.data);
                    var intl = new ej2_base_3.Internationalization(this.parent.locale);
                    var value = intl.formatNumber((sizeValue / 1024), { format: 'n' });
                    modifiedSize = value + ' ' + utility_1.getLocaleText(this.parent, 'KB');
                }
                sizeEle.innerHTML = modifiedSize;
            }
            if (this.parent.isMobile) {
                if (ej2_base_1.getValue('_fm_modified', args.data) !== undefined && args.row.querySelector('.e-fe-date')) {
                    ...
                    var formattedString = intl.formatDate(new Date(ej2_base_1.getValue('_fm_modified', args.data)), format);
            ...
            if (!index_2.hasEditAccess(args.data)) {
                args.row.className += ' ' + index_2.getAccessClass(args.data);
            }
            var eventArgs = {
                element: args.row,
            ...
            if (args.requestType === 'sorting') {
                ...
                if (this.parent.selectedItems.length !== 0) {
                    ...
                    while (len > 0) {
                        ...
                        this.sortSelectedNodes.push(ej2_base_1.getValue(this.parent.hasId ? 'id' : 'name', data));
                        len--;
                    }
                }
        ...
        DetailsView.prototype.onBeforeDataBound = function (args) {
            ej2_popups_1.showSpinner(this.parent.element);
            // eslint-disable-next-line
            var items = utility_1.getSortedData(this.parent, this.gridObj.dataSource);
            args.result = items;
        };
        /* istanbul ignore next */
        DetailsView.prototype.onDataBound = function () {
            ...
            if (this.parent.createdItem) {
                this.selectRecords([ej2_base_1.getValue(this.parent.hasId ? 'id' : 'name', this.parent.createdItem)]);
            ...
            if (this.gridObj.currentViewData.length * this.gridObj.getRowHeight() < this.gridObj.height) {
                ...
                var hdContent = ej2_base_1.select('.e-headercontent', hdTable);
        ...
        DetailsView.prototype.selectRecords = function (nodes) {
            ...
            for (var i = 0, len = gridRecords.length; i < len; i++) {
                var node = this.parent.hasId ? ej2_base_1.getValue('id', gridRecords[i]) : index_1.getName(this.parent, gridRecords[i]);
        ...
        DetailsView.prototype.addSelection = function (data) {
            ...
            if (this.parent.hasId) {
                rData = new ej2_data_1.DataManager(items).
                    executeLocal(new ej2_data_1.Query().where('id', 'equal', this.parent.renamedId, false));
            }
            else {
                // eslint-disable-next-line
                var nData = new ej2_data_1.DataManager(items).
                    executeLocal(new ej2_data_1.Query().where('name', 'equal', ej2_base_1.getValue('name', data), false));
                if (nData.length > 0) {
                    rData = new ej2_data_1.DataManager(nData).
        ...
        DetailsView.prototype.onSortColumn = function () {
            if (this.parent.sortOrder !== 'None') {
                ...
                // eslint-disable-next-line
                this.gridObj.dataSource = utility_1.getSortedData(this.parent, this.gridObj.dataSource);
            }
        };
            ...
            for (var _i = 0, _a = Object.keys(e.newProp); _i < _a.length; _i++) {
                ...
                switch (prop) {
                    ...
                    case 'detailsViewSettings':
                        if (!ej2_base_1.isNullOrUndefined(this.gridObj)) {
                            this.checkNameWidth();
                            var columns = this.getColumns();
                    ...
                    case 'selectedItems':
                        ...
                        else if (!ej2_base_2.isNullOrUndefined(this.gridObj)) {
                    ...
                    case 'allowMultiSelection':
                        ...
                        break;
                    case 'view':
                        utility_1.updateLayout(this.parent, 'Details');
                        break;
                    case 'width':
        ...
        DetailsView.prototype.onPathChanged = function (args) {
            ...
            utility_1.removeBlur(this.parent);
            if (this.parent.view === 'Details') {
                /* istanbul ignore next */
                this.isInteracted = false;
                ej2_popups_1.showSpinner(this.parent.element);
                this.parent.setProperties({ selectedItems: [] }, true);
                this.gridObj.dataSource = utility_1.getSortedData(this.parent, args.files);
            }
            this.emptyArgs = args;
        };
        DetailsView.prototype.updatePathColumn = function () {
            ...
            if (columnData[len - 1].field && columnData[len - 1].field !== 'filterPath' && !this.parent.isMobile) {
                var pathColumn = {
                    field: 'filterPath', headerText: utility_1.getLocaleText(this.parent, 'Path'), minWidth: 180, width: 'auto'
                };
        ...
        DetailsView.prototype.checkEmptyDiv = function (args) {
            // eslint-disable-next-line
            var items = utility_1.getSortedData(this.parent, args.files);
            if (items.length === 0 && !ej2_base_2.isNullOrUndefined(this.element.querySelector('.' + CLS.GRID_VIEW))) {
        ...
        DetailsView.prototype.openContent = function (data) {
            var _this = this;
            if (!index_2.hasReadAccess(data)) {
                index_2.createDeniedDialog(this.parent, data, events.permissionRead);
                return;
            }
            ...
            this.parent.trigger('fileOpen', eventArgs, function (fileOpenArgs) {
                if (!fileOpenArgs.cancel) {
                    var name_2 = ej2_base_1.getValue('name', data);
                    if (ej2_base_1.getValue('isFile', data)) {
                        var icon = utility_1.fileType(data);
                        if (icon === CLS.ICON_IMAGE) {
                            var imgUrl = utility_1.getImageUrl(_this.parent, data);
                            dialog_1.createImageDialog(_this.parent, name_2, imgUrl);
                        }
                    }
                    else {
                        var val = _this.parent.breadcrumbbarModule.searchObj.element.value;
                        if (val === '' && !_this.parent.isFiltered) {
                            var id = ej2_base_1.getValue('id', data);
                            var newPath = _this.parent.path + (ej2_base_2.isNullOrUndefined(id) ? name_2 : id) + '/';
                            _this.parent.setProperties({ path: newPath }, true);
                            _this.parent.pathNames.push(name_2);
                            _this.parent.pathId.push(ej2_base_1.getValue('_fm_id', data));
                            _this.parent.itemData = [data];
                            utility_1.openAction(_this.parent);
                        }
                        else {
                            index_1.openSearchFolder(_this.parent, data);
                        }
        ...
        DetailsView.prototype.onSelectionChanged = function () {
            ej2_base_2.removeClass([this.element], CLS.HEADER_CHECK);
            if (this.parent.selectedItems.length > 0) {
                ej2_base_2.addClass([this.element], CLS.HEADER_CHECK);
            }
        };
        ...
        DetailsView.prototype.onPasteInit = function () {
            if (this.parent.activeModule === this.getModuleName()) {
                this.parent.itemData = (this.parent.folderPath !== '') ? this.gridObj.getSelectedRecords() :
                    [index_1.getPathObject(this.parent)];
            }
        };
        DetailsView.prototype.onDetailsInit = function () {
            if (this.parent.activeModule === this.getModuleName()) {
                if (this.parent.selectedItems.length !== 0) {
                    ...
                    this.parent.itemData = [ej2_base_1.getValue(this.parent.pathId[this.parent.pathId.length - 1], this.parent.feParent)];
                }
            }
        };
        ...
        DetailsView.prototype.onDetailsResize = function () {
            if (this.parent.view === 'Details' && !this.parent.isMobile && !ej2_base_2.isNullOrUndefined(this.gridObj)) {
        ...
        DetailsView.prototype.createDragObj = function () {
            ...
            if (!this.parent.isMobile && this.gridObj) {
                if (this.parent.allowDragAndDrop) {
                    ...
                    this.dragObj = new ej2_base_3.Draggable(this.gridObj.element, {
        ...
        DetailsView.prototype.onDropInit = function (args) {
            if (this.parent.targetModule === this.getModuleName()) {
                /* istanbul ignore next */
                // eslint-disable-next-line
                var cwdData = ej2_base_1.getValue(this.parent.pathId[this.parent.pathId.length - 1], this.parent.feParent);
                if (!args.target.closest('tr')) {
                    ...
                    this.parent.dropPath = info.isFile ? this.parent.path : index_3.getFullPath(this.parent, info, this.parent.path);
        ...
        DetailsView.prototype.onpasteEnd = function (args) {
            if (this.parent.view === 'Details') {
                ...
                if (this.parent.path === this.parent.destinationPath || this.parent.path === index_4.getDirectoryPath(this.parent, args)) {
                    this.onPathChanged(args);
                }
            }
        };
        ...
        DetailsView.prototype.onSelected = function (args) {
            ...
            if (!ej2_base_2.isNullOrUndefined(item) && !ej2_base_2.isNullOrUndefined(item.querySelector('.e-checkselect'))) {
                if (this.gridObj.getSelectedRowIndexes().length !== 1) {
            ...
            if (!ej2_base_2.isNullOrUndefined(this.gridObj) && !ej2_base_2.isNullOrUndefined(this.gridObj.element.querySelector('.e-checkselectall'))) {
            ...
            if (len > 0) {
                // eslint-disable-next-line
                var data = this.gridObj.getRowsObject()[rows[len - 1]].data;
                this.parent.currentItemText = ej2_base_1.getValue('name', data);
            }
            var indexes = ej2_base_1.getValue('rowIndexes', args);
            var multiSelect = ej2_base_1.getValue('enableSelectMultiTouch', this.gridObj.selectionModule);
            if (this.parent.isDevice && ej2_base_2.isNullOrUndefined(indexes) && args.target && !multiSelect && !args.target.closest('.e-headercell')) {
                this.parent.isFile = ej2_base_1.getValue('isFile', args.data);
                if (!this.parent.isFile) {
                    this.openContent(args.data);
                }
            }
            this.parent.visitedItem = args.row;
            if (this.parent.allowMultiSelection && !ej2_base_2.isNullOrUndefined(item) && !ej2_base_2.isNullOrUndefined(item.querySelector('.e-checkselect'))) {
        ...
        DetailsView.prototype.onPathColumn = function () {
            if (this.parent.view === 'Details' && !ej2_base_2.isNullOrUndefined(this.gridObj)) {
                if (this.parent.breadcrumbbarModule.searchObj.element.value === '' && !this.parent.isFiltered) {
                    this.removePathColumn(false);
                }
            }
        ...
        DetailsView.prototype.selectedRecords = function () {
            ...
            while (selectSize < selectedRecords.length) {
                ...
                var name_3 = index_4.getItemName(this.parent, record);
        ...
        DetailsView.prototype.onDeSelection = function (args) {
            /* istanbul ignore next */
            if (!this.parent.allowMultiSelection && ej2_base_2.isNullOrUndefined(args.data)) {
            ...
            this.selectedRecords();
            if (this.parent.selectedItems.length === 0) {
                ej2_base_2.setValue('enableSelectMultiTouch', false, this.gridObj.selectionModule);
                ej2_base_2.removeClass([this.parent.element], CLS.MULTI_SELECT);
            }
            this.parent.notify(events.selectionChanged, {});
        ...
        DetailsView.prototype.wireEvents = function () {
            ...
            this.keyboardModule = new ej2_base_1.KeyboardEvents(this.gridObj.element, {
            ...
            ej2_base_1.EventHandler.add(this.gridObj.element, 'blur', this.removeFocus, this);
        };
        DetailsView.prototype.unWireEvents = function () {
            this.wireClickEvent(false);
            this.keyboardModule.destroy();
            this.keyboardDownModule.destroy();
            ej2_base_1.EventHandler.remove(this.gridObj.element, 'blur', this.removeFocus);
        };
        DetailsView.prototype.wireClickEvent = function (toBind) {
            if (toBind) {
                ...
                this.clickObj = new ej2_base_2.Touch(ele, {
                    tap: function (eve) {
                    ...
                    tapHold: function (e) {
                        if (proxy_1.parent.isDevice) {
                            ...
                            if (proxy_1.parent.allowMultiSelection) {
                                ej2_base_2.setValue('enableSelectMultiTouch', proxy_1.parent.allowMultiSelection, proxy_1.gridObj.selectionModule);
                                ej2_base_2.addClass([proxy_1.parent.element], CLS.MULTI_SELECT);
                            }
                            var target = e.originalEvent.target;
                            if (target) {
                                var row = ej2_base_3.closest(target, '.' + CLS.ROW);
        ...
        DetailsView.prototype.getFocusedItemIndex = function () {
            return (!ej2_base_2.isNullOrUndefined(this.getFocusedItem())) ?
        ...
        DetailsView.prototype.keyupHandler = function (e) {
            ...
            var action = e.action;
            // eslint-disable-next-line
            var gridItems = utility_1.getSortedData(this.parent, this.gridObj.dataSource);
            ...
            switch (action) {
                case 'altEnter':
                    ...
                    break;
                case 'esc':
                    index_1.removeActive(this.parent);
                    break;
                case 'del':
                case 'shiftdel':
                    this.performDelete();
                ...
                case 'enter':
                    ...
                    break;
                case 'ctrlC':
                    index_1.copyFiles(this.parent);
                    break;
                case 'ctrlV':
                    this.parent.folderPath = '';
                    index_1.pasteHandler(this.parent);
                    break;
                case 'ctrlX':
                    index_1.cutFiles(this.parent);
                    break;
                case 'ctrlD':
                    this.doDownload();
                    break;
                case 'f2':
                    ...
                    break;
                case 'ctrlA':
                    if (!ej2_base_2.isNullOrUndefined(gridItems[0]) && this.parent.allowMultiSelection) {
                        var cnTable = this.gridObj.getContent().querySelector('.e-content');
                ...
                case 'ctrlHome':
                case 'tab':
                    if (!ej2_base_2.isNullOrUndefined(gridItems[0])) {
                ...
                case 'csEnd':
                    if (!this.parent.allowMultiSelection) {
                        ...
                        if (!ej2_base_2.isNullOrUndefined(gridItems[0])) {
                            if (!ej2_base_2.isNullOrUndefined(selIndex) && selIndex !== -1) {
                ...
                case 'ctrlDown':
                    ...
                    break;
                case 'home':
                    firstItem = [ej2_base_1.getValue(this.parent.hasId ? 'id' : 'name', gridItems[0])];
                ...
                case 'moveDown':
                    ...
                    break;
                case 'end':
                    lastItem = [ej2_base_1.getValue(this.parent.hasId ? 'id' : 'name', gridItems[gridLength - 1])];
        ...
        DetailsView.prototype.doDownload = function () {
            if (this.parent.selectedItems.length !== 0) {
                ...
                for (var i = 0; i < items.length; i++) {
                    if (!index_2.hasDownloadAccess(items[i])) {
                        index_2.createDeniedDialog(this.parent, items[i], events.permissionDownload);
                        return;
                    }
                }
                operations_1.Download(this.parent, this.parent.path, this.parent.selectedItems);
            }
        };
        DetailsView.prototype.performDelete = function () {
            if (this.parent.selectedItems && this.parent.selectedItems.length > 0) {
                ...
                var items = this.parent.itemData;
                for (var i = 0; i < items.length; i++) {
                    if (!index_2.hasEditAccess(items[i])) {
                        index_2.createDeniedDialog(this.parent, items[i], events.permissionEdit);
                        return;
                    }
                }
        ...
        DetailsView.prototype.moveFunction = function (selectedItems, e, rowIndex) {
            if (!ej2_base_2.isNullOrUndefined(this.getFocusedItem()) && this.parent.allowMultiSelection) {
            ...
            else if (!ej2_base_2.isNullOrUndefined(rowIndex) && rowIndex !== -1) {
                ...
                if (!ej2_base_2.isNullOrUndefined(selectedItems[0])) {
                    this.gridObj.selectRow(0);
                }
            }
        };
        DetailsView.prototype.spaceSelection = function (selRowIndeces, focIndex, selIndex, e) {
            ...
            else if (!ej2_base_2.isNullOrUndefined(this.getFocusedItem()) && focIndex !== selIndex) {
        ...
        DetailsView.prototype.ctrlMoveFunction = function (items, e, rowIndex) {
            var nextItem;
            if (!ej2_base_2.isNullOrUndefined(this.getFocusedItem())) {
                ...
                if (!ej2_base_2.isNullOrUndefined(items[0])) {
                    nextItem = 0;
                }
            }
        ...
        DetailsView.prototype.checkRowsKey = function (items, indexValue, focIndex, e) {
            if (this.gridObj.checkAllRows === 'Uncheck' || this.gridObj.checkAllRows === 'Intermediate') {
                if (e.action !== 'csHome' && e.action !== 'csEnd') {
                    if (ej2_base_2.isNullOrUndefined(this.startIndex) && this.firstItemIndex !== indexValue) {
                    ...
                    if (e.action === 'csEnd') {
                        this.gridObj.
                            selectRows(this.InnerItems(ej2_base_2.isNullOrUndefined(indexValue) ? 0 : indexValue, ej2_base_2.isNullOrUndefined(focIndex) ? items.length - 1 : focIndex, e));
                    }
                    else {
                        if (ej2_base_2.isNullOrUndefined(indexValue)) {
                            this.gridObj.selectRow(0);
                        }
                        else {
                            this.gridObj.selectRows(this.InnerItems(ej2_base_2.isNullOrUndefined(focIndex) ? 0 : focIndex, indexValue, e));
                        }
                    }
                }
            }
        ...
        DetailsView.prototype.shiftSelectFocusItem = function (selIndex, fIndex, selRowIndexes, e) {
            ...
            var fItem = ej2_base_2.isNullOrUndefined(this.startIndex) ? selIndex : selRowIndexes[0];
            if (fItem === lItem) {
        ...
        DetailsView.prototype.addFocus = function (item) {
            ...
            if (fItem) {
                ...
                ej2_base_2.removeClass([fItem], [CLS.FOCUS, CLS.FOCUSED]);
            }
            if (!ej2_base_2.isNullOrUndefined(itemElement)) {
                this.gridObj.element.setAttribute('tabindex', '-1');
                itemElement.setAttribute('tabindex', '0');
                itemElement.focus();
                ej2_base_2.addClass([itemElement], [CLS.FOCUS, CLS.FOCUSED]);
            }
        };
        DetailsView.prototype.getFocusedItem = function () {
            return ej2_base_1.select('.' + CLS.FOCUSED, this.element);
        };
        ...
        DetailsView.prototype.shiftSelectedItem = function (selIndex, selRowIndexes, gridItems, e) {
            if (selIndex === -1) {
                ...
                if (ej2_base_2.isNullOrUndefined(this.startIndex) && e.shiftKey) {
                    this.startIndex = this.gridObj.selectedRowIndex;
        ...
        DetailsView.prototype.onMethodCall = function (e) {
            ...
            var action = ej2_base_1.getValue('action', e);
            switch (action) {
                case 'deleteFiles':
                    this.deleteFiles(ej2_base_1.getValue('ids', e));
                    break;
                case 'downloadFiles':
                    this.downloadFiles(ej2_base_1.getValue('ids', e));
                    break;
                case 'openFile':
                    this.openFile(ej2_base_1.getValue('id', e));
                    break;
                case 'createFolder':
                    this.interaction = false;
                    break;
                case 'renameFile':
                    this.interaction = false;
                    this.renameFile(ej2_base_1.getValue('id', e), ej2_base_1.getValue('newName', e));
                    break;
                case 'selectAll':
                    this.interaction = false;
        ...
        DetailsView.prototype.getRecords = function (nodes) {
            ...
            if (this.parent.hasId || !hasFilter) {
                for (var i = 0, len = gridRecords.length; i < len; i++) {
                    if (nodes.indexOf(ej2_base_1.getValue(filter, gridRecords[i])) !== -1) {
                ...
                for (var i = 0, len = gridRecords.length; i < len; i++) {
                    var name_4 = ej2_base_1.getValue('filterPath', gridRecords[i]) + ej2_base_1.getValue('name', gridRecords[i]);
        ...
        DetailsView.prototype.deleteFiles = function (ids) {
            ...
            for (var i = 0; i < records.length; i++) {
                data[i] = records[i];
                newIds[i] = index_4.getItemName(this.parent, data[i]);
            }
        ...
        DetailsView.prototype.renameFile = function (id, name) {
            ...
            var records = this.getRecords([id]);
            if (records.length > 0) {
                index_4.updateRenamingData(this.parent, records[0]);
                if (!ej2_base_2.isNullOrUndefined(name)) {
                    if (index_2.hasEditAccess(this.parent.itemData[0])) {
                        index_2.rename(this.parent, this.parent.path, name);
                    }
                    else {
                        index_2.createDeniedDialog(this.parent, this.parent.itemData[0], events.permissionEdit);
                    }
                }
                else {
                    index_2.doRename(this.parent);
